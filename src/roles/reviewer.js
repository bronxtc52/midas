...
function reviewerPrompt({ issue, plan, diff, rubric }) {
  return [
    ...
    'У тебя НЕТ Bash и запуска команд: runtime-пункты DoD (npm test, запуск скриптов/линтеров) НЕ выполняй и не пытайся — ревью запускается только после зелёного CI-гейта, считай такие пункты подтверждёнными CI и проверяй остальное статически по диффу и коду.',
