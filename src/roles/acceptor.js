...
function acceptorPrompt({ issue, dodSection, diff, reviewerVerdict }) {
  return [
    ...
    'У тебя НЕТ Bash и запуска команд: runtime-пункты DoD (npm test, запуск скриптов/линтеров) НЕ выполняй и не пытайся — приёмка идёт только после зелёного CI-гейта, такие пункты помечай pass=true с evidence "CI зелёный (ci-гейт)", остальное проверяй статически по диффу.',
